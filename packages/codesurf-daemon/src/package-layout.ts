...
function safePathSegments(value: string): boolean {
...
function normalizeRuntimeEntry(value: unknown): string {
 ...
 */
export function getDaemonRuntimeEntries(manifest: DaemonPackageManifest): string[] {
...
export function getDaemonCompiledExports(
  ...
  for (const [subpath, rawTarget] of Object.entries(manifest.exports)) {
    ...
    const target = rawTarget as Partial<DaemonPackageExportTarget>
    if (
...
export function getDaemonPublicSpecifiers(
  manifest: DaemonPackageManifest,
  packageName = '@codesurf/daemon',
): string[] {
  if (!packageName || packageName.endsWith('/')) {
    throw new Error('daemon package name must be a non-empty bare specifier')
  }
  return getDaemonCompiledExports(manifest).map(target => (
    target.subpath === '.' ? packageName : `${packageName}${target.subpath.slice(1)}`
  ))
}

import { readDataProperty, singleLine, utf8Bytes, utf8Prefix, } from './peer-context-serialization.js';
import { getAllNodeToolNames } from './node-tools.js';
...
function boundedId(value) {
    ...
    if (!id || utf8Bytes(id) > MAX_ID_BYTES || !SAFE_ID.test(id))
        return null;
    if (id === '.' || id === '..' || id.includes('..') || id.endsWith('.'))
        return null;
    return id;
}
...
export function resolveAuthoritativeCanvasPeers(canvas, tileIdValue) {
    const tileId = boundedId(tileIdValue);
    const rawTiles = data(canvas, 'tiles');
    const rawConnections = data(canvas, 'lockedConnections');
    if (!tileId || !Array.isArray(rawTiles) || !Array.isArray(rawConnections))
        return [];
    ...
    for (let index = 0; index < Math.min(rawTiles.length, MAX_CANVAS_TILES); index += 1) {
        ...
        if (!id || tileTypes.has(id))
            continue;
        tileTypes.set(id, boundedType(data(tile, 'type')));
    }
    if (!tileTypes.has(tileId))
        return [];
    ...
    for (let index = 0; index < Math.min(rawConnections.length, MAX_CANVAS_CONNECTIONS); index += 1) {
        ...
        if (peerId && tileTypes.has(peerId))
            peerIds.add(peerId);
    }
    return [...peerIds]
        .sort()
...
export function getAuthoritativeNegotiatedPeerTools(peers, mcpEnabled) {
    if (mcpEnabled === false)
        return undefined;
    return [...new Set(peers.flatMap(peer => peer.tools))].sort();
}
/** Keep caller observations only for peer IDs already proven by the canvas. */
export function selectAuthorizedPeerObservations(value, authoritativePeers) {
    if (!Array.isArray(value) || authoritativePeers.length === 0)
        return [];
    ...
    for (let index = 0; index < value.length && selected.length < authoritativePeers.length; index += 1) {
        ...
        if (!peerId || !allowed.has(peerId) || seen.has(peerId))
            continue;
        seen.add(peerId);

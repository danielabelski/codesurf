...
function parseOverlay(value, index) {
    ...
    for (const [field, maxBytes] of Object.entries(textLimits)) {
        if (Object.hasOwn(input, field)) {
            ;
            output[field] = boundedString(input[field], `personas[${index}].${field}`, maxBytes);
        }
    }
    ...
    if (Object.hasOwn(input, 'skills')) {
        ...
        output.skills = input.skills.map((skill, skillIndex) => boundedString(skill, `personas[${index}].skills[${skillIndex}]`, 128).trim());
    }
...
export async function listAuthoritativePersonas(workspaceRoot) {
    ...
    try {
        return overlayAuthoritativePersonas(JSON.parse(await readBoundedFile(join(root, '.codesurf', 'customisation', 'agents.json'))));
    }
    catch {
        return DEFAULT_PERSONAS.map(persona => ({ ...persona }));
    }
}
export function bindChatRequestToWorkspace(request, workspace) {

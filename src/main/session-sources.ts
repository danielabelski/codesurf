...
async function listPiAgentSessions(workspacePath: string | null): Promise<AggregatedSessionEntry[]> {
  const candidateFiles = new Map<string, string | null>()
...
async function parsePiAgentChatState(filePath: string, entry: AggregatedSessionEntry): Promise<ImportedChatState | null> {
  const messages: ImportedChatMessage[] = []
  let model = entry.model
